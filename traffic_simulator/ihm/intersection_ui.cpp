...
/**
  ...
  */
IntersectionUI::IntersectionUI(QGraphicsItem *parent)
...
/**
  ...
  */
void IntersectionUI::addRoads(const Map::ListeRoutes &routes)
{
  //Remplir les variables m_Points et m_Segments avec le contenu de routes
  foreach(Map::Road* r, routes)
  {
    if (r->getPremiereIntersection() && r->getDeuxiemeIntersection())
    {
      ...
      if (r->getPremiereIntersection()->nom() == m_Name)
      {
...
/**
  ...
  */
void IntersectionUI::drawIntersection()
{
...
void IntersectionUI::setNameIntersection(const QString &nom)
{
  m_Name = nom;
}

void IntersectionUI::setCentreIntersection(const Utils::Point &centre)
{
  m_Centre = centre;
}
